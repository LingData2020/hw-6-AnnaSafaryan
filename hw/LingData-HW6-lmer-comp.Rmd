...
```{r 1.1, echo = FALSE}
ggplot(upsid, aes(consonants, vowels)) +
  theme_bw() +
  geom_point() 
```
...
ggplot(upsid, aes(consonants, vowels)) +
  theme_bw() +
  geom_point() +
  geom_line(data = augment(fit1), aes(consonants, .fitted), color = "blue")
```
...
ggplot(upsid, aes(consonants, vowels, color = area)) + 
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit2), aes(consonants, .fitted, color = area))
```
...
```{2.4}
Переменная consonansts всё ещё значима, причём p-value повысился с добавлением area random effect (0.000905 > 1.63e-06). Линии параллельны и отличаются только смещением -- она определяется соответствующим значением area, к которым был добавлен случайный эффект.
```
...
ggplot(upsid, aes(consonants, vowels, color = area)) + 
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit3), aes(consonants, .fitted, color = area))
...
```{3.3}
p-value настолько увеличилось (=0.48), что мы больше не можем назвать consonants значимым параметром. Линии при этом больше не параллельны, т.к. мы добавили случайный эффект к наклону -- возможно, именно это повлияло на изменение значимости consonants: случайность наклона перевесила значения этого параметра. Зато они с одинаковым смещением, т.е. начинаются из одной точки независимо от параметра area
```
...
ggplot(upsid, aes(consonants, vowels, color = area)) + 
  geom_point() +
  theme_bw() +
  geom_line(data = augment(fit4), aes(consonants, .fitted, color = area))
```
...
```{4.3}
Переменная consonants всё ещё не значима, хотя её p-value немного понизилась по сравнению с предыдущим (0.41 < 0.48). При этом, линии теперь имеют как разный наклон, так и разное смещение. Разница в предсказаниях модели объясняется тем, к чему мы добавляем случайный эффект: 
  1) первая модель, изменяющая смещение, указывает на важность ореоола для определения количества гласных
  2) вторая модель, изменяющая наклон, показывает, что существуют разные зависимости между количеством гласных и количеством согласных
  3) третья модель, объединяющая подходы, позволяет учесть и ореол, и количество согласных
